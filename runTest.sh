...
start_database() {
  ...
  ${BENCHMARK}/startdb.sh $DB $RESOURCE_USAGE_PATH $DBFOLDER
}

stop_database() {
  #ssh -n -T ${RUSER}@${TEST_IP} ${BENCHMARK}/stopdb.sh $DBFOLDER
  ${BENCHMARK}/stopdb.sh $DBFOLDER
}

TODAY=`date +%Y.%m.%d`
TESTS_INCLUDE_WARMUP=warmup
TESTS_EXCLUDE_WARMUP=singleWrite,singleWriteSync,singleRead,neighbors,neighbors2,neighbors2data,aggregation,shortest,hardPath
